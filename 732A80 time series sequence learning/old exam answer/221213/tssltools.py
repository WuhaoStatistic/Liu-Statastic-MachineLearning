...
def theta_update_wrong(ks):
    """ Function for updating the weights in EM algorithm

    :param ks: results from the kalman smoother. Of class kfs_res

    :return theta: new updates of the two theta values.
    """
    sigma2_eps = np.mean(ks.eta_cov)
    sigma2_eta = np.mean(ks.eps_var)
    return np.array([sigma2_eta, sigma2_eps])
...
def propagate_wrong(particles, b):
	"""Function for propagating the particles
	...
	return b + np.random.randn(size = particles.size) 
