---
title: "computational statistic lab2"
author: "Wuhao Wang"
date: "11/13/2021"
...
The 'use_optim()' function takes there parameters: 
...
for each interval, this function will call 'use_optim()' to find the optimized
parabolic. When all the interval are approximated,this function will print the 
graph of comparasion between original function and new piecewise function.
```{r}
...
```
question 3:
The piece wise parabolic fitted very well.
```{r }
f1 <- function(x)
{
  return(-x*(1-x))
...
It's a bad idea to maximize likelihood function, since this process is related to exponential computing, which is very time consuming
and will use much memory.
```{r }
...
All the algorithms converge in all cases. These 4 algorithms return the same value of optimal parameters.
so optimal u = 1.275528, sigma = 2.005977.
```{r }
print(bf[['par']])
```
For CG method:
...
print(bf[['counts']])
```
Since all the algorithms can get same results,I think BFGS method without a given gradient is good, just because of
fewer evaluations.
